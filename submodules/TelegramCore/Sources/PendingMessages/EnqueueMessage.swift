...
private func telewhiteStrippedResendableMedia(_ media: Media) -> Media? {
    if let image = media as? TelegramMediaImage {
        // Drop `reference` so uploadedMediaImageContent cannot take the
        // inputMediaPhoto-by-reference shortcut; representations keep pointing at
        // the cached resources so the bytes are found locally and re-uploaded.
        ...
        // Strip the CloudDocumentMediaResource fileReference so the
        // inputMediaDocument-by-reference shortcut is skipped and the file is
        // re-uploaded from cache.
...
public func telewhiteResendableMessagesFromProtectedSource(_ message: Message) -> [EnqueueMessage]? {
    ...
    for media in message.media {
        if media is TelegramMediaImage || media is TelegramMediaFile {
            ...
            // Not re-sendable as a fresh upload; skip and fall back to text.
            continue
        } else {
            return nil
        }
    ...
    for (index, media) in resendableMedia.enumerated() {
        // Only the first media item carries the caption, matching how Telegram
        // groups an album with a single shared caption.
