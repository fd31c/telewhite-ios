...
- **[Добавлено]** Автоп��бликация обновлений: после каждой сборки воркфлоу создаёт GitHub Release
...
- **[Исправлено]** `95dbbdd` — Кнопка **Show Original** в панели перевода тепер�� работает: клик
  переключает перевод даже без Premium (когда включён авто-перевод Telewhite), а выбор пользователя
  «показать ор��гинал» больше не перезатирается авто-переводом при повторном входе в чат
...
- **[Улучш��но]** `f72a706` — При включённом `hidePhoneInSettings` вместо номера телефона показывается «—».
