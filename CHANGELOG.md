...
- **[Возвращено]** Глобальный режим невидимки (откат `f0e137f`). В `Telewhite Mods → Ghost Mode` возвращён тогл
  «Global Ghost Mode / Глобальный режим невидимки»: `isGhostEnabled(for:)` снова возвращает `true` для всех чатов
  при включённом `ghostMode` (скрытие прочтений и набора текста везде), а `shouldKeepOnlinePresence` снова
  учитывает `ghostMode` наряду с `hideOnlineStatus` и `ghostPeerIds`. Per-chat кнопка-призрак в навбаре чата
  сохраняется и работает как раньше: при включённом глобальном режиме тап по ней конвертирует глобальный ghost
  в per-chat для этого чата. Файл: `submodules/SettingsUI/Sources/TelewhiteModsController.swift`.
