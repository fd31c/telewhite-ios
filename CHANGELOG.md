...
- **[Улучшено]** Пока призрак включён хотя бы в одном чате, аккаунт удерживается офлайн и после отправки ответа повторно подтверждает серверу статус «был(а) недавно» каждые 5 секунд. После выключения последнего призрака обычный online возвращается.
